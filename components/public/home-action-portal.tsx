...
import { ArrowRight, FileSignature, SearchCheck, Send, Ticket } from "lucide-react";
import { ActionModal } from "./action-modal";
import { ComplaintForm } from "./complaint-form";
import { TrackingForm } from "./tracking-form";
import { RecentTicketBanner } from "./recent-ticket-banner";
...
export function HomeActionPortal() {
  ...
  const handleTrackFromBanner = (code: string) => {
    setTrackTicketCode(code);
    ...
    <>
      <div className="grid gap-6 sm:grid-cols-2">
        ...
        <div
          ...
          onKeyDown={(e) => (e.key === "Enter" || e.key === " ") && setActiveModal("complaint")}
        ...
        >
          ...
          <div className="mt-8 flex items-center justify-between border-t border-stone-100 pt-5">
            <span className="text-sm font-bold text-[#1f4f8f]">Buka Formulir Aduan</span>
        ...
        <div
          ...
          onKeyDown={(e) => (e.key === "Enter" || e.key === " ") && setActiveModal("tracking")}
          className="group relative flex flex-col justify-between cursor-pointer overflow-hidden rounded-2xl border border-stone-200 bg-white p-7 text-left shadow-lg transition-all duration-200 hover:-translate-y-1.5 hover:border-[#1f4f8f] hover:shadow-2xl sm:p-8"
        >
          ...
          <div className="mt-8 flex items-center justify-between border-t border-stone-100 pt-5">
            <span className="text-sm font-bold text-[#1f4f8f]">Cek Status Aduan</span>
      ...
      {/* Banner Tiket Terakhir jika baru saja kirim dan modal tertutup */}
      <RecentTicketBanner onTrack={handleTrackFromBanner} />
      ...
      <ActionModal
        ...
        maxWidth="2xl"
      >
        <TrackingForm initialTicket={trackTicketCode} key={trackTicketCode} />
      </ActionModal>
    </>
  );
}
